...
package com.google.mlkit.vision.QReveIonic.java.barcodescanner;
...
import com.google.mlkit.vision.QReveIonic.ResultadoLeido;
import com.google.mlkit.vision.QReveIonic.java.Constant;
import com.google.mlkit.vision.QReveIonic.java.VisionProcessorBase;
...
public class BarcodeScannerProcessor extends VisionProcessorBase<List<Barcode>> {
  ...
  protected void onSuccess(
    ...
    if (barcodes.isEmpty()) {
      Log.v("MIO", "Java - No barcode has been detected");
    } else {
      Log.v("MIO", "Cantidad de qrs: " + barcodes.size());
    }

    boolean leido = Constant.leido;

    if (!leido) {
      for (int i = 0; i < barcodes.size(); ++i) {
        Log.i("MIO", "ENCONTRE UN QR: " + barcodes);
        Barcode barcode = barcodes.get(i);
        String valor = barcode.getRawValue();
        Constant.valor_leido = valor;
        Log.i("MIO", "El QR leido es: " + valor);
      ...
      if (Constant.leido) {
        // barcodeScanner.close();
        Log.i("MIO", "YA NO LEO MAS");

        Intent i = new Intent(mcontext, ResultadoLeido.class);
